//////////////////////////////////////////////////////////////////////////////
//                                                                          //
//  * Autor: PEDRO MELENDEZ DEL POZO - 53404                                //
//                                                                          //
//  * Archivo: APDS9960.cpp                                                 //
//                                                                          //
//  * Tema:  TFG - Implementación de un sistema de control de persianas     //
//                 basado en gestos con integración domótica.               //
//                                                                          //
//  * Descripcion: Adaptación de la librería del sensor APDS 9960 para      //
//                 su integración con microcontroladores STM32.             //
//                                                                          //
//  * Versión: 1.0                                                          //
//                                                                          //
//////////////////////////////////////////////////////////////////////////////
...
bool APDS9960::init()
{
	...
	if( !setGestureGain(GGAIN_1X) ) {//DEFAULT_GGAIN
		return false;
	}
...
int APDS9960::readGesture()
{
	...
	while(1)
	{
		...
		if( (gstatus & APDS9960_GVALID) == APDS9960_GVALID )
		{
			...
			{
				sprintf((char*)msg," %d ",fifo_data[i]);
				HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);
			}
#endif
			...
			if( bytes_read >= 4 )
			{
				...
				{
					sprintf((char*)msg," %d ",gesture_data_.u_data[i]);
					HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);
				}
#endif
...
bool APDS9960::readAmbientLight(uint16_t &val)
{
	...
	if( !APDS_9960_Read(APDS9960_CDATAL, val_byte) )
	{
		return false;
	}
	val = val_byte;
	...
	if( !APDS_9960_Read(APDS9960_CDATAH, val_byte) )
	{
		return false;
	}
...
bool APDS9960::readRedLight(uint16_t &val)
{
	...
	/* Read value from clear channel, low byte register */
	if( !APDS_9960_Read(APDS9960_RDATAL, val_byte) )
	{
		return false;
	}
	val = val_byte;
	...
	/* Read value from clear channel, high byte register */
	if( !APDS_9960_Read(APDS9960_RDATAH, val_byte) )
	{
		return false;
	}
	val = val + ((uint16_t)val_byte << 8);
...
bool APDS9960::readGreenLight(uint16_t &val)
{
	...
	/* Read value from clear channel, low byte register */
	if( !APDS_9960_Read(APDS9960_GDATAL, val_byte) )
	{
		return false;
	}
	val = val_byte;
	...
	/* Read value from clear channel, high byte register */
	if( !APDS_9960_Read(APDS9960_GDATAH, val_byte) )
	{
		return false;
	}
	val = val + ((uint16_t)val_byte << 8);
...
bool APDS9960::readBlueLight(uint16_t &val)
{
	...
	/* Read value from clear channel, low byte register */
	if( !APDS_9960_Read(APDS9960_BDATAL, val_byte) )
	{
		return false;
	}
	val = val_byte;
	...
	/* Read value from clear channel, high byte register */
	if( !APDS_9960_Read(APDS9960_BDATAH, val_byte) )
	{
		return false;
	}
	val = val + ((uint16_t)val_byte << 8);
...
bool APDS9960::readProximity(uint8_t &val)
{
	...
	if( !APDS_9960_Read(APDS9960_PDATA, val) )
	{
		return false;
	}
...
bool APDS9960::processGestureData()
{
			...
			(gesture_data_.total_gestures > 0) )
	{
		...
		for( i = gesture_data_.total_gestures - 1; i >= 0; i-- )
		{
			...
			sprintf((char*)msg," U: %d ",gesture_data_.u_data[i]);
			HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);

			sprintf((char*)msg," D: %d ",gesture_data_.d_data[i]);
			HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);

			sprintf((char*)msg," L: %d ",gesture_data_.l_data[i]);
			HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);
	...
	sprintf((char*)msg,"\n Last Values: ");
	HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);
	sprintf((char*)msg," U: %d   D: %d   L: %d   R: %d",u_last,d_last,l_last,r_last);
	HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);

	sprintf((char*)msg,"\n Ratios: ");
	HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);
	sprintf((char*)msg," UD Fi: %d   UD La: %d   LR Fi: %d   LR La: %d",ud_ratio_first,ud_ratio_last,lr_ratio_first,lr_ratio_last);
	HAL_UART_Transmit(&huart2,(uint8_t*)msg,strlen(msg),HAL_MAX_DELAY);
...
uint8_t APDS9960::getProxIntLowThresh()
{
	...
	if( !APDS_9960_Read(APDS9960_PILT, val) )
	{
		val = 0;
	}
...
uint8_t APDS9960::getProxIntHighThresh()
{
	...
	if( !APDS_9960_Read(APDS9960_PIHT, val) )
	{
		val = 0;
	}
...
uint8_t APDS9960::getProxGainCompEnable()
{
	...
	if( !APDS_9960_Read(APDS9960_CONFIG3, val) )
	{
		return ERROR;
	}
...
bool APDS9960::setProxGainCompEnable(uint8_t enable)
{
	...
	if( !APDS_9960_Write(APDS9960_CONFIG3, val) )
	{
		return false;
	}
...
uint8_t APDS9960::getGestureEnterThresh()
{
	...
	if( !APDS_9960_Read(APDS9960_GPENTH, val) )
	{
		val = 0;
	}
...
uint8_t APDS9960::getGestureExitThresh()
{
	...
	if( !APDS_9960_Read(APDS9960_GEXTH, val) )
	{
		val = 0;
	}
...
bool APDS9960::getLightIntLowThreshold(uint16_t &threshold)
{
	...
	if( !APDS_9960_Read(APDS9960_AILTL, val_byte) )
	{
		return false;
	}
	threshold = val_byte;
	...
	if( !APDS_9960_Read(APDS9960_AILTH, val_byte) )
	{
		return false;
	}
...
bool APDS9960::getLightIntHighThreshold(uint16_t &threshold)
{
	...
	if( !APDS_9960_Read(APDS9960_AIHTL, val_byte) )
	{
		return false;
	}
	threshold = val_byte;
	...
	if( !APDS_9960_Read(APDS9960_AIHTH, val_byte) )
	{
		return false;
	}
	threshold = threshold + ((uint16_t)val_byte << 8);
...
bool APDS9960::getProximityIntLowThreshold(uint8_t &threshold)
{
	...
	if( !APDS_9960_Read(APDS9960_PILT, threshold) )
	{
		return false;
	}
...
bool APDS9960::getProximityIntHighThreshold(uint8_t &threshold)
{
	...
	/* Read value from proximity low threshold register */
	if( !APDS_9960_Read(APDS9960_PIHT, threshold) )
	{
		return false;
	}
...
uint8_t APDS9960::getAmbientLightIntEnable()
{
	...
	if( !APDS_9960_Read(APDS9960_ENABLE, val) )
	{
		return ERROR;
	}
...
bool APDS9960::setAmbientLightIntEnable(uint8_t enable)
{
	...
	if( !APDS_9960_Write(APDS9960_ENABLE, val) )
	{
		return false;
	}
...
{
	uint8_t throwaway;
	if( !APDS_9960_Read(APDS9960_AICLEAR, throwaway) )
	{
		return false;
	}
...
{
	uint8_t throwaway;
	if( !APDS_9960_Read(APDS9960_PICLEAR, throwaway) )
	{
		return false;
	}
